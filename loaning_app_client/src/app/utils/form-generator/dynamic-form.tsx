...
const DynamicForm = ({ formRef, onSubmit }: { formRef: React.RefObject<HTMLFormElement | null>, onSubmit : (data:Record<string,unknown>) => void}) => {
  const { selectedForm, generateYupSchema, setFormErrors, formDatas } = useFormGen();
  const schema = useMemo(() => generateYupSchema(selectedForm?.fields), [generateYupSchema, selectedForm?.fields]);

  const defaultValues = selectedForm?.fields?.reduce((accu: {[key:string] : string | Date | null}, field) => {
    const fieldValue = formDatas[field.id] ?? "";
    if(field.component === 'date' && fieldValue) accu[field.id] = dayjs(fieldValue as string).isValid() ? dayjs(fieldValue as string).toDate() : null;
    ...
    <form onSubmit={handleSubmit(onSubmit)} ref={formRef} style={{ height: 'auto', width: '100%', padding: 0, margin: 0}} >
      ...
      {selectedForm?.fields?.map((field, index) => {
       const Component = fieldMap[field.component as keyof typeof fieldMap]; 
        ...
        return <Grid2 size={field.width} key={index}><Component key={index} control={control} errors={errors} {...register(field.id)} {...field} /></Grid2>;
      })}
      </Grid2>
