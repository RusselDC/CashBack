...
const Dropdown = ({id, label, style, control, errors, dropdownOpts, apiId, parentAddress, objectAddress} : CustomDropdownProps) => {
    ...
    render={({field}) => (
        ...
        <Select
          ...
          onChange={(newValue) => {
            ...
            setDropdownValue((prev) => ({ ...prev, [objectAddress as string]: newValue.target.value as string }))
          }}
        ...
        >
        {
            options ? Object.keys(options).map((key,index) => {
                ...
                return <MenuItem key={`${options?.[key]}-${index}`} value={options?.[key]}>{key}</MenuItem>}) : <MenuItem>NO DATA</MenuItem>
        }
        </Select>
        {errors[id]?.message && (
